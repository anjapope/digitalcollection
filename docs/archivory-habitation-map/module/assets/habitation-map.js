...
async function open(id){panel.hidden=false;try{await loadState(id);setTimeout(()=>map?.resize(),0)}catch(e){console.error("ArchIvory habitation map error:",e);q("[data-ahm-status]").textContent="The habitation map could not be loaded.";}}
function close(){panel.hidden=true;emit("archivory:habitation-map-close",{source:"habitation-map"});}
...
window.addEventListener("archivory:timeline-state-change",e=>{if(!panel.hidden&&e.detail?.mapState)loadState(e.detail.mapState).catch(console.error);});
fetch(STATES_URL).then(r=>{if(!r.ok)throw new Error(`Map-state request failed (${r.status})`);return r.json()}).then(d=>states=d).catch(e=>{console.error("ArchIvory map-state error:",e);q("[data-ahm-status]").textContent="Map configuration could not be loaded.";});
